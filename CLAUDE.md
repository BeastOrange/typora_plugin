...
Typora Plugin is an extensible plugin system for the Typora Markdown editor. It injects into Typora's Electron-based runtime (via a modified `window.html`) and provides 50+ plugins. Pure JavaScript (no TypeScript), requires Typora >= 0.9.98, supports Windows and Linux.
...
- `settings.default.toml` (~120KB) — Default config for all base plugins. Each plugin has a `[plugin_name]` section with `ENABLE`, `NAME`, and plugin-specific options.
...
- **AOP**: `decorator.js` wraps Typora internals without modifying source. Used by `eventHub`, `exportHelper`, and many plugins.
